...
<section id="<?= $dom_id; ?>" class="relative bg-darkGray text-white py-16 lg:py-24 overflow-hidden border-b-4 border-primary">
    ...
    <div class="absolute inset-0 bg-cover bg-center opacity-15" style="background-image: url('<?= $hero_image; ?>');"></div>
            ...
            <div class="bg-white rounded-lg p-6 sm:p-8 text-slate-800 shadow-2xl border-t-4 border-primary">
                ...
                <form id="heroLeadForm" class="space-y-4">
                    ...
                    <div>
                        ...
                        <textarea id="lead_message" rows="3" placeholder="Deskripsikan luas dapur atau lokasi usaha Anda..." class="w-full px-3 py-2 text-sm border border-gray-300 rounded focus:ring-2 focus:ring-primary focus:border-primary outline-none transition"></textarea>
                    </div>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        if (heroForm) {
            heroForm.addEventListener('submit', function(e) {
                ...
                const waText = `*KONSULTASI DAPUR KOMERSIAL*\n\n` +
                    `• *Nama/PIC:* ${name}\n` +
                    `• *No. WhatsApp:* ${phone}\n` +
                    `• *Kebutuhan:* ${service}\n` +
